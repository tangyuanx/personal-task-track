...
> 最近核对：2026-07-18，面向 v0.1.74。
>
> 本文使用函数名和文件职责定位，不再维护容易漂移的行号。查找实现时优先运行
> `rg -n "function 函数名|data-action=\"动作名\"" src electron scripts`。

## 1. 架构

```text
index.html
├─ src/styles.css                 主题、布局、响应式、无障碍状态
├─ src/vendor/milkdown-editor.*  由 esbuild 生成的 Milkdown 浏览器资源
└─ src/app.js                    渲染进程的状态、DOM、事件和业务逻辑
   ├─ electron/preload.cjs       安全 contextBridge
   ├─ electron/main.cjs          BrowserWindow、IPC、剪贴板、导出
   └─ electron/storage.cjs       数据规范化、损坏备份、原子读写
```
...
| 文件 | 职责 |
|------|------|
| `src/app.js` | 全部渲染进程状态、查询、渲染、事件、CRUD、回顾、Markdown、导出编排 |
| `src/styles.css` | 浅色/深色主题、桌面/窄屏布局、组件状态、键盘焦点、减少动效 |
| `src/milkdown-editor.entry.js` | Milkdown/Crepe 封装、持久化图片上传、图片命令 |
| `electron/main.cjs` | 窗口安全配置、IPC、系统剪贴板、Markdown/PDF 导出 |
| `electron/preload.cjs` | 向渲染进程暴露最小化存储、剪贴板和导出 API |
| `electron/storage.cjs` | 任务数据规范化、旧字体迁移、损坏 JSON 备份、原子写入 |
| `scripts/test.cjs` | Node 回归测试与渲染脚本 VM 测试夹具 |
| `scripts/build-milkdown.cjs` | 构建 Milkdown JS/CSS |
| `scripts/build.cjs` | 调用 electron-builder；外置卷自动使用本机临时输出；复制发布产物并清理 AppleDouble |
| `scripts/after-pack.cjs` | 清理应用包内 AppleDouble 文件 |
...
- `src/milkdown-editor.entry.js` 将粘贴图片保存为 data URL，避免 blob URL 在重载后失效。
...
| 主题资源 | `src/styles.css` 中 `:root` 与 `:root[data-theme="dark"]` |
...
`npm run check` 会重建 Milkdown、执行关键脚本语法检查并运行 `scripts/test.cjs`。

在 `/Volumes` 外置工作区中，`scripts/build.cjs` 会自动把 ASAR 构建输出切换到系统临时目录，再复制最终发布文件回 `release/`；CI 和普通磁盘工作区仍使用默认输出目录。
