...
(async () => {
  ...
  try {
    ...
    await trigger.click();
    const firstPersonalGroup = page.locator('.repository-group-option[data-personal-group="true"]').first();
    assert.equal(await firstPersonalGroup.count(), 1, "expected a personal group in the clean profile");
    const groupTitle = (await firstPersonalGroup.locator("span").first().innerText()).trim();
    await firstPersonalGroup.click();
