...
test("release version, scripts and cache keys are aligned", () => {
  assert.equal(pkg.version, "0.1.191");
  assert.match(html, /work-navigation-model\.js\?v=0\.1\.179/);
  assert.match(html, /work-rhythm\.css\?v=0\.1\.191/);
  assert.match(html, /work-rhythm\.js\?v=0\.1\.191/);
  assert.match(html, /styles\.css\?v=0\.1\.191/);
  assert.match(html, /app\.js\?v=0\.1\.191/);
});
