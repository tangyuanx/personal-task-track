...
test("deadline reminder stages ignore undated and completed tasks", () => {
  ...
  assert.equal(deadlineReminderStage({ status: "active", deadlineAt: "2026-08-25T01:00:00.000Z" }, now), "due-day");
  assert.equal(deadlineReminderStage({ status: "active", deadlineAt: "2026-08-24T03:00:00.000Z" }, now), "due-soon");
...
test("successful reminders deduplicate across scans and application restarts", async () => {
  ...
  const tasks = [{ id: "task-a", title: "完成日历功能", status: "active", deadlineAt: "2026-08-24T03:00:00.000Z" }];

  await sync({}, tasks);
  await first.controller.run();
  assert.equal(FakeNotification.instances.length, 1);
  assert.equal(FakeNotification.instances[0].options.title, "任务即将截止");

  first.setNow("2026-08-24T04:00:00.000Z");
  await first.controller.run();
  await first.controller.run();
  assert.equal(FakeNotification.instances.length, 2);
  assert.equal(FakeNotification.instances[1].options.title, "任务已逾期");
  await first.controller.stop();

  const persisted = JSON.parse(await fs.readFile(reminderStateFilePath(directory), "utf8"));
  assert.deepEqual(persisted.tasks["task-a"].stages.sort(), ["due-soon", "overdue"]);
...
test("notification clicks return to one task or the calendar for grouped reminders", async () => {
  ...
  await harness.controller.sync([
    { id: "task-b", title: "任务 B", status: "active", deadlineAt: "2026-08-24T03:30:00.000Z" },
    { id: "task-c", title: "任务 C", status: "active", deadlineAt: "2026-08-24T03:45:00.000Z" },
  ]);
