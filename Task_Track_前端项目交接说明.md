...
- v0.1.47 候选已在本地完成：知识笔记页签复用已挂载的 Milkdown 实例、状态与更新时间固定为处理流末列、状态视觉改为低对比绿色文本轨道、右键状态菜单互斥、空分组侧栏不再产生大面积空白；macOS ARM64 与 Windows x64 安装包均已成功构建，等待提交、标签和远程发布；
- 本文档从本次提交开始纳入 Git，并作为后续会话的持续交接入口；
- `task-track.html` 是用户提供的视觉/交互基线文件，目前仍是本地参考文件，不等于生产入口；
- `.design-qa/` 是本地截图和比较证据目录，不属于生产运行依赖。

如果后续工作已经发生，必须在结束前更新这一小节，不能让“当前任务状态”长期停留在 v0.1.47 候选。
...
- `npm run check` 与 `git diff --check` 已通过；macOS ARM64 DMG/ZIP、Windows x64 NSIS 安装包均已构建，发布目录不存在 `._*` 污染文件；尚待提交、标签和远程推送。
